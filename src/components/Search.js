...
const Search = ({ mode = "light", onSearchModalToggle, user }) => {
  ...
  // Memoized values
  const userTier = useMemo(() => normalizeTier(user?.selected_tier || "Free Member"), [user?.selected_tier]);
        ...
        const results = await Promise.all(
          tables.map(async ({ name, section }) => {
            ...
            const filteredItems = data
              .filter((item) => normalizeTier(item.tier_restriction || "Free Member") <= userTier)
              .map((item) => ({
                id: item.id,
                title: item.title,
