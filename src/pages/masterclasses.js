...
import { useMasterclasses, useMasterclassRegistrations, useMasterclassCategories } from '@/hooks/useMasterclasses';
...
export default function MemberMasterclasses() {
  ...
  // Use the useMasterclassCategories hook for categories
  const { 
    categories = [], 
  ...
  // Use the useMasterclasses hook for available masterclasses
  const { 
    masterclasses: availableMasterclasses, 
  ...
  // Use the useMasterclassRegistrations hook for enrolled masterclasses
  const { 
    registrations: enrolledMasterclasses, 
    loading: loadingEnrolled, 
    error: enrolledError,
    refetch: refetchEnrolled 
  } = useMasterclassRegistrations({
    user_id: user?.id,
    ...(filters.status && { status: filters.status })
  });
  
  const loading = loadingAvailable || loadingEnrolled || loadingCategories;

  // Refresh enrolled masterclasses when user or status filter changes
  useEffect(() => {
    if (user?.id) {
      refetchEnrolled();
    }
  }, [user, filters.status, refetchEnrolled]);
                          ...
                          <div className="mt-4 flex items-center justify-between">
                            ...
                            <div className="flex items-center space-x-2">
                              {isUpcoming(registration.masterclass?.start_date) && (
                                <Link
                                  href={`/masterclasses/${registration.masterclass?.id}`}
