          ...
          <div className="absolute bottom-4 left-4 right-4">
            <div className="flex items-center justify-between">
              <div className="flex space-x-3">
                {modalData?.is_available && !isRegistered && (
                  <button
                    onClick={handleBookNow}
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-paan-blue hover:bg-paan-blue/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-paan-blue"
                  >
                    <Icon icon="mdi:calendar" className="mr-2 -ml-1 h-4 w-4" />
                    Book Now
                  </button>
                )}
                {modalData?.registration_link && (
                  <a
                    href={modalData.registration_link}
                    target="_blank"
    ...
    <div>
      ...
      <div className="flex flex-wrap gap-3">
        {tags.map((tag, index) => (
          ...
          >
            <Icon icon="mdi:tag" className="text-paan-yellow text-sm mr-2" />
            {tag}
          </span>
        ))}
      </div>
...
const UpdateModalContent = ({ modalData, mode, onClose }) => {
  ...
  if (modalData.cta_url) {
    actions.push({
      ...
      onClick: () =>
        window.open(modalData.cta_url, "_blank", "noopener,noreferrer"),
...
const ResourceModalContent = ({ modalData, mode, onClose }) => {
  ...
  if (modalData.file_path) {
    actions.push({
      ...
      onClick: () =>
        window.open(modalData.file_path, "_blank", "noopener,noreferrer"),
      color: "bg-paan-blue hover:bg-paan-blue/80",
    });
  ...
  return (
    <div className="space-y-8">
      ...
      <InfoGrid mode={mode}>
        {(modalData.city || modalData.country) && (
          <InfoCard
            icon="mdi:map-marker"
            label="Location"
            value={[modalData.city, modalData.country]
              .filter(Boolean)
              .join(", ")}
            iconColor="text-paan-yellow"
            iconBgColor={mode === "dark" ? "bg-yellow-900/30" : "bg-yellow-50"}
      ...
      <DescriptionSection
        ...
        description={stripHtml(modalData.description)}
        mode={mode}
      />
      ...
      <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700">
        <button
          onClick={onClose}
          className={`px-6 py-2.5 text-sm font-medium rounded-xl border transition-all duration-200 hover:shadow-xl transform hover:-translate-y-0.5 ${
            mode === "dark"
              ? "border-gray-600 text-gray-200 bg-gray-800 hover:bg-gray-700 hover:border-gray-500"
              : "border-gray-200 text-gray-700 bg-white hover:bg-gray-50 hover:border-gray-300"
          }`}
        ...
        {modalData.is_available && !isRegistered && (
          <button
            onClick={handleBookNow}
            className={`px-6 py-2.5 text-sm font-medium rounded-xl text-white transition-all duration-200 bg-paan-blue hover:paan-blue/60 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 ${
              mode === "dark" ? "shadow-white/10" : "shadow-gray-200"
            }`}
          >
        ...
        {!isRegistered && modalData.registration_link && (
          <a
            href={modalData.registration_link}
            target="_blank"
            rel="noopener noreferrer"
            className="px-6 py-2.5 text-sm font-medium rounded-xl text-white transition-all duration-200 bg-gradient-to-r from-paan-yellow to-yellow-500 hover:from-yellow-500 hover:to-paan-yellow shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center space-x-2"
          >
        ...
        {/* Registered Status - Only show if registered */}
        {isRegistered && (
          <div className="px-6 py-2.5 text-sm font-medium rounded-xl text-white bg-green-500 flex items-center space-x-2">
...
const SimpleModalContent = ({ modalData, mode, user, onClose }) => {
  return (
    <div className="space-y-6">
      <div className="text-center">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-paan-yellow/20 mb-4">
          <Icon icon="mdi:clock-outline" className="w-8 h-8 text-paan-yellow" />
        </div>
        <h3
      ...
      <div className="bg-gradient-to-r from-paan-blue/10 to-paan-dark-blue/10 rounded-2xl p-6 border border-paan-blue/20">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            >
              We'll notify you as soon as this feature becomes available. Keep
              an eye on your email and the PAAN platform for updates.
            </p>
          </div>
        </div>
      </div>
...
const UnifiedModalContent = ({
  ...
  const modalProps = {
    ...
    ...((modalData.type === "awardCategories" ||
      modalData.type === "nominateCampaign") && {
      user,
    }),
  };
